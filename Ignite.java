...
public class Ignite extends SimState{
	...
	/* simulation params */
	public int numUAVs = 2; //number of mavs involved in the simulation, try with 3 - ok, 4 - ok
	...
	public static int cellsOnFire = 0; // static, will be problem, convert to a function
	...
	public static boolean isInBounds(Double3D pos){
        ...
        public int cellsOnFire() { // will be used in fireController to update the knowledge about the fires dynamically
            int cof = 0; // cells on fire initially 0
            for(int i=0; i<width; i++){
				for(int j=0; j<height; j++){
					WorldCell cell = (WorldCell) forest.field[i][j];
					if(cell.type.equals(CellType.FIRE)) // if the given cells is on fire
						cof++; // increase the count
			}
		}
            return cof;
        }
	...
	public void start(){
		...
		UAVs = new Bag(); // to add the drones later
		...
		for(int l=0; l<lakes; l++){
			...
			int radius = 1; // to be considered in the logic, necessary for decision making!!!
		...
		//generate fires
		int fires = 3; // test with 4, 5, 6, 10, RAMPAGE!!!!!!!!!!!!
		...
		for(int i = 0 ; i < numUAVs; i++){
			...
			UAVs.add(uav); //so we can access all the drones separately
